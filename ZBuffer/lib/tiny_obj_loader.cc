  ...
  const std::string &name)
{
  ...
  for (size_t i = 0; i < faceGroup.size(); i++) {
    ...
    for (size_t k = 2; k < npolys; k++) {
      ...
      indices.push_back(v0);  // index of this vertex in positions array
  ...
  const char* mtl_basepath)
{
  ...
  std::vector<float> vt;         // texture vertices
